...
5.  **【裝備屬性智慧判定鐵律】**: 你必須根據物品的類型與名稱，為其設定「equipSlot」、「hands」、「bulk」三個新欄位。
    * **\`equipSlot\` (佩掛位置)**: 你必須從以下列表中，為物品選擇一個**最合理**的佩掛位置。
        * **武器佩掛位置**: "weapon_right" (佩在右腰), "weapon_left" (佩在左腰), "weapon_back" (背在身後)。請根據武器的常規佩戴習慣選擇，例如單刀通常在腰間，長劍或弓可背在身後。
        * **防具飾品位置**: "head", "body", "hands", "feet", "accessory1", "accessory2", "manuscript"。
        * 如果物品完全不可裝備（如消耗品、材料），則此欄位值為 \`null\`。
    * **\`hands\` (武器手數)**: **只有**當 \`itemType\` 是「武器」時，才需要設定此欄位。你必須根據武器名稱判斷。單手武器為 \`1\`，雙手武器為 \`2\`。例如：「長劍」是1，「巨劍」是2。
