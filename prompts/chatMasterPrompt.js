...
const getChatMasterPrompt = (npcProfile, chatHistory, playerMessage, longTermSummary = '（目前沒有需要參考的江湖近況）', localLocationContext = null, mentionedNpcContext = null) => {
    ...
    // 【核心修改】將 remoteLocationContext 更名為 mentionedNpcContext 並修改內容
    const mentionedNpcInstruction = mentionedNpcContext
        ? `\n## 你正在被詢問的「另一位NPC」的情報檔案 (此為最高優先級情報)：
        你必須優先參考此檔案，並結合你自己的檔案中與他/她的關係，來生成你的回應。
        \`\`\`json
        ${JSON.stringify(mentionedNpcContext, null, 2)}
        \`\`\``
        : '';
...
4.  **【極重要】情報區分與情境感知**：
    ...
    * 當玩家詢問你**其他NPC**的事情時（即「另一位NPC的情報檔案」有內容時），你**必須**切換到一個「**知情者**」的口吻。你的回答**必須**基於你與他/她的關係。例如，孫女提到祖母時應稱呼「奶奶」或「祖母」，而不是直呼其名。**絕對禁止**像讀報告一樣直接複述對方情報。
