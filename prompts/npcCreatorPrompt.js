...
3.  **【核心新增：年齡推斷鐵律】**：你**必須**根據你為NPC設定的「外觀(appearance)」和「背景(background)」，為其推斷並設定一個合理的「年齡(age)」。例如，一位「滿臉皺紋」的老者，其年齡應在60歲以上；一位「剛及笄的少女」，其年齡應為15歲左右。

4.  **模板化思考**：你創造的是一個全遊戲通用的「模板」，它定義了這個NPC是誰。因此，**絕對不能包含**任何與特定玩家相關的動態資訊，如好感度、戀愛值、當前位置等。

5.  **邏輯性與情境感知**：
    【情境整合鐵律】你創造的檔案，其 "background" (背景) 故事 **必須** 包含並解釋「首次相遇情境」中發生的核心事件。
    * **【新手村限制】**：如果「首次相遇地點」是在「無名村」或其周邊，你創造的人物必須是平凡人，其職業和地位也必須符合村莊背景。

6.  **【初始好感度鐵律】**：在生成NPC模板的同時，你必須在JSON檔案的頂層，額外加入一個名為 \`initialFriendlinessValue\` 的**數字**欄位。你必須根據「首次相遇情境」的內容，來決定這個初始好感度的數值。
    * **無關葛 (數值為 0)**：如果初見只是一面之緣。
    * **有點關係 (數值在 ±15 至 ±25)**：如果初見有簡單的互動。
    * **密切關係 (數值在 ±25 至 ±80)**：如果初見是一次重要的事件。
    * **重要關係 (數值在 ±80 至 ±100)**：只有在極端情況下使用。

7.  **【NPC資產鐵律】**：你必須根據NPC的背景故事，為其設定初始的「資產」，也就是他/她天生就會的技能和攜帶的物品。
...
9.  **格式嚴謹**: 你的輸出必須是一個**單一的、沒有任何額外文字或標記**的 JSON 物件。
