...
2.  **邏輯性與情境感知**：
    ...
    * **【新手村限制】如果「首次相遇地點」是在「無名村」或其周邊地區，你創造的人物必須是平凡的。** 其 "skills" (技能) 欄位只能包含生活技能（如：打鐵、採藥、烹飪）或完全不包含此欄位。其 "background" (背景) 必須是普通人的背景，**絕對禁止設定任何與權力核心（如朝廷高官）、頂尖組織（如武林盟主、最大商會）或超凡能力相關的隱藏身份。**
    * 只有當相遇地點是在大城市（如：開封、臨安）或特定的門派地點時，你才可以設定其為真正的江湖人士。
3.  **【核心新增】初始好感度鐵律**：在生成NPC模板的同時，你必須在JSON檔案的頂層，額外加入一個名為 \`initialFriendlinessValue\` 的**數字**欄位。你必須根據「首次相遇情境」的內容，來決定這個初始好感度的數值。
    * **無關葛 (數值為 0)**：如果初見只是一面之緣，例如問路或擦肩而過，則設為0。
    * **有點關係 (數值在 ±15 至 ±25)**：如果初見有簡單的互動，例如一次普通的交易、一次簡短的善意提醒或一次輕微的冒犯。
    * **密切關係 (數值在 ±25 至 ±80)**：如果初見是一次重要的事件，例如NPC救了玩家、玩家完成了NPC的重要委託、或是雙方發生了激烈但非生死的衝突。
    * **重要關係 (數值在 ±80 至 ±100)**：只有在極端情況下使用，例如NPC是玩家的殺父仇人或救命恩人，兩人的命運從一開始就緊緊綁定。
4.  **【核心新增】NPC資產鐵律**：你必須根據NPC的背景故事，為其設定初始的「資產」，也就是他/她天生就會的技能和攜帶的物品。
...
5.  **關係定義鐵律**：
    * **單向定義**: 在生成 "relationships" 欄位時，你**只需要定義當前NPC（${npcName}）與他人的單向關係**。例如，若要設定 ${npcName} 是「李鐵匠」的兒子，你只需在 ${npcName} 的檔案中寫入 \`"父親": "李鐵匠"\`。你**不需要**，也**不應該**去思考或修改「李鐵匠」的檔案，後續的雙向關係將由系統自動處理。
...
6.  **格式嚴謹**: 你的輸出必須是一個**單一的、沒有任何額外文字或標記**的 JSON 物件。
