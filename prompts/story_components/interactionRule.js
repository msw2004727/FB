...
2.  **【核心升級：盟友協同作戰判斷鐵律】**:
    * 在判定一方發起攻擊後，你**必須**立刻掃描現場所有NPC，判斷他們的陣營歸屬。
    * **判定盟友 (Ally Judgment)**: 如果現場有NPC符合以下**任一條件**，你**必須**將其判定為盟友：
        * **友好度達標**: 該NPC對玩家的友好度為 'friendly', 'trusted', 'devoted'。
        * **明確指令**: 玩家的指令中明確包含了協同作戰的關鍵詞，如：「**我和(盟友名)**」、「**跟(盟友名)一起**」、「**我們上**」、「幫我」、「並肩作戰」、「掩護我」。
        * **捨身取義**: 該NPC的個性(personality)包含「忠誠」、「義氣」、「勇敢」，且當前情境是玩家被圍攻或處於劣勢，此時NPC應會主動參戰。
    * **判定敵人 (Enemy Judgment)**: 確認最終的敵對目標。
    * **進入戰鬥**: 如果最終確定將進入交鋒，你必須在回傳的 \`roundData\` 中，額外加入以下欄位：
        1.  \`"enterCombat": true\`
        2.  \`"combatants"\`: 一個包含所有**對手**姓名和初始狀態的物件陣列。
        3.  \`"allies"\`: 一個包含所有參戰**盟友**姓名和初始狀態的物件陣列。如果沒有符合條件的盟友，則**不要**包含此欄位。
        4.  \`"combatIntro"\`: (字串) 一段約50-100字的文字，生動地描述戰鬥發生的原因、敵我陣容或場景氛圍。
    * **協同作戰範例**:
        * **玩家輸入**: "我對金小麥說：『我們一起上！』，隨後攻向陳奎。"
        * **你應回傳的JSON片段**:
          \`\`\`json
          "enterCombat": true,
          "combatants": [{"name": "陳奎", "status": "準備應戰"}],
          "allies": [{"name": "金小麥", "status": "點頭示意，準備協同攻擊"}],
          "combatIntro": "你一聲令下，金小麥心領神會，兩人交換了一個眼神，分從左右兩路，一同向陳奎攻了過去！"
          \`\`\`
