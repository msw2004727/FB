...
5. "roundData" 鍵的值是一個物件，必須包含以下所有欄位：
    ...
    - CLS: (字串) 關鍵線索
    - IMP: (字串) 行動造成的直接影響
6. 【死亡判定規則】如果故事的發展對玩家造成了不可逆轉的致命後果（例如：被利刃刺穿心臟、服下劇毒且無解藥、墜入萬丈深淵），或者玩家的行動明確表達了自殺的意圖（例如「拔劍自刎」、「跳崖自盡」），你必須在 "story" 中描述其死亡的結局，並將 "playerState" 欄位的值設為 "dead"。在所有其他情況下，此值都應為 "alive"。
