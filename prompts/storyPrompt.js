...
-   在回傳的 \`NPC\` 陣列中，你只需要提供一個包含該角色**最基本資訊**的物件。
-   **【嚴格規則】** 這個物件**只能**包含四個鍵：\`"name"\`、\`"status"\`、\`"friendliness"\`，以及最重要的**\`"isNew": true\`**。
...
3.  **心動值變化判斷**：你的回傳資料中，\`roundData\` 物件**必須**包含一個名為 \`romanceChanges\` 的**陣列**。如果沒有任何NPC的心動值發生變化，請回傳一個**空陣列 \`[]\`**。
    * 陣列中的每一個物件，都代表一位NPC的心動值變化，其結構必須如下：
      ...
      \`\`\`
    * **正面行為範例**：捨身保護、贈送極具心意的禮物、觸動其內心的對話、完成其個人任務等，應產生**正值**變化（如 \`15\`）。
...
現在，請根據以上的長期摘要、世界觀、規則（特別是最新的「新手保護規則」、物品帳本系統）、最近發生的事件和玩家的最新行動，生成下一回合的JSON物件。
`;
};
