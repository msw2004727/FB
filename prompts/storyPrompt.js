...
## 戰鬥模式規則 (非常重要)：
當劇情發展到一場**結構化的戰鬥或對決**時，無論是生死相搏還是點到為止的比試，你都必須觸發戰鬥系統。
- **觸發時機**: 當玩家的行動明確表示要與人動手時，例如「攻擊山賊」、「與林教頭切磋一下」、「跟李大哥比試武藝」、「教訓這個惡霸」等。
- **觸發方式**: 你必須在回傳的 \`roundData\` 中，額外加入兩個欄位：
    1.  \`"enterCombat": true\`
    2.  \`"combatants"\`: 一個包含所有對手姓名和初始狀態的**物件陣列**。例如：\`[{"name": "山賊頭目", "status": "氣焰囂張"}, {"name": "林教頭", "status": "抱拳以待"}]\`
- **例外**: 如果對手實力與玩家相差過於懸殊（例如玩家欺負不會武功的平民），則應直接在故事中描述結果，而**不要**觸發戰鬥系統。
