...
const getAIPostCombatResultPrompt = (playerProfile, finalCombatState, combatLog) => {
    const { player, enemies, allies, intention } = finalCombatState; // 【核心修改】讀取意圖
...
- **劇情**: 這是一場生死鬥，你的描述必須體現其殘酷性。
    - **若玩家勝利**: 你必須根據敵人的**個性**和**身份**，決定一個合理的致命結局。一個悍匪可能會被梟首，一個求饒的懦夫可能被你一念之仁放過（但依然重傷），一個高手可能在留下遺言後氣絕身亡。
