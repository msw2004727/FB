...
- Updated current-location sidebar card label and button tooltip in `scripts/uiUpdater.js` to Chinese (`≤Œ™v™Ã`, `¨d¨›¶a∞œ∏‘±°`) and restored Chinese loading/fallback text.
- Updated location-details modal labels/section titles in `scripts/modalManager.js` to Chinese, including summary/static/dynamic section headers and field-name mappings (`√˛´¨`, `¶aß}`, `ºhØ≈∏ÙÆ|`, `≤Œ™v™Ã`, etc.).
- Restored visible fallback strings in the modal from English (`N/A`, `Unknown`) to Chinese (`µL`, `•º™æ`) to avoid mixed-language UI in the current-location feature.
...
ame fields and use Chinese fallback strings (•º™æ¶a∞œ, ¶a∞œ±°≥¯∏¸§J§§...) to reduce placeholder leakage into UI.
- Validation: 
