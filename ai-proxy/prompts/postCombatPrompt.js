...
${combatLog.slice(-5).join('\n')}
    ...
    return `
你是一位絕對中立、邏輯嚴謹的「戰後結算AI」。一場激烈的戰鬥剛剛結束，你的唯一任務是根據以下的「戰鬥總結報告」與「核心裁決鐵律」，生成一個結構化的JSON物件，用以記錄本次戰鬥的最終「實質結果」。

**【最高優先級禁令】**: 你的回應中，絕對禁止包含任何 "narrative" (故事描述) 欄位。你的職責是提供數據，而非故事。

**【語言鐵律】**: 你的所有回應文字（包括 summary、EVT、PC 等所有文字欄位）必須全程使用「繁體中文」。嚴格禁止簡體中文。
...
- **實質結果**: 這是一場生死鬥，你的裁決必須體現其殘酷性。
    - **戰利品 ('itemChanges')**: 只有在**敵人死亡**的情況下，才**可能**會掉落物品。掉落的物品必須與其身份相符（例如：敵人掉落金錢和粗製的兵器，富商掉落銀票）。掉落機率和品質由你根據合理性判斷。
    - **NPC狀態 ('npcUpdates')**:
...
{
  "outcome": {
    "summary": "一段約150-250字的、符合現代小說風格的戰後場景描述。這段文字將作為新回合的故事主體。你必須生動地描寫戰鬥結束後的場景、角色的神態、以及事件的直接後果。例如，如果敵人死亡，描寫你如何收劍、查看屍體、以及周遭的反應。如果玩家戰敗，描寫你倒下時的感受和看到的最後一幕。",
