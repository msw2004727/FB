...
6.  "roundData" key 的值是一個物件，必須包含以下所有欄位：
    ...
    - actionMorality: (陣列) **必填**。對應 actionOptions 的 3 個善惡值。規則：
      - 善行（助人、正義、保護弱者）→ 正數（如 5~15）
      - 惡行（偷竊、傷害、欺騙、挑釁、背叛）→ 負數（如 -5~-15）
      - 中性行為（觀察、移動、休息）→ 0
      - **【鐵律】三個選項中至少要有一個是「反骨選項」（負數），代表邪道、冒險或道德灰色的行動。禁止三個選項全部為 0 或全部為正數。**
