...
2.  **【語言鐵律 — 最高優先級】你的所有回應文字（包括 story、EVT、PC、PSY、CLS、WRD、LOR、IMP 等所有欄位），必須全程使用「繁體中文」。嚴格禁止：簡體中文、英文單字、日文、emoji 表情符號、任何非繁體中文字元。違反此規則等同任務失敗。**
...
6.  "roundData" key 的值是一個物件，必須包含以下所有欄位：
    ...
    - ATM: (陣列) [氛圍, 感官細節]
      - "ATM[0]"（氛圍）必須是簡短標籤，最多8個字；禁止完整句子（例如可用「壓抑緊繃」、「熱鬧喧騰」）
    - EVT: (字串) 事件摘要
    ...
    - LOC: (陣列) 【核心修改】玩家當前的位置，採用層級結構。例如：["無名村"] (代表在村莊的公共區域，如廣場)，或是 ["無名村", "藥鋪"] (代表在藥鋪內)。
    - PSY: (字串) 角色內心獨白或感受
    - PC: (字串) 玩家狀態變化
    ...
    - claimBounty: (可選的物件)
    - mentionedLocations: (可選的陣列)
    - locationUpdates: (可選的陣列)
