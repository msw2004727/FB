...
6.  "roundData" key 的值是一個物件，必須包含以下所有欄位：
    ...
    - actionOptions: (陣列) 恰好 3 個下一步行動，每個 8-15 個繁體中文字、以動詞開頭，且符合當前世界觀。
    - actionMorality: (陣列) 與 actionOptions 一一對應的 3 個數字，至少包含一個正數與一個負數。
    - suggestion: (字串) 15 字以內的實用建議或吐槽。
    - progressEval: (物件) 必須包含：
      - triggered: (布林值) 只有故事明確符合「下一個里程碑」的觸發條件時才可為 true。
      - reason: (字串) 20 字以內的嚴格判定理由。
      - questJournal: (字串) 30 字以內的目前主線感知狀態。
