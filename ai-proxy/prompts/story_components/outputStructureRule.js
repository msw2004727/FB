...
5.  "story" 鍵的值是一個字串，用來生動地描述故事發展，字數控制在300字以內，精煉為主。
6.  "roundData" key 的值是一個物件，必須包含以下所有欄位：
    ...
    - actionOptions: (陣列) **必填**。恰好 3 個字串，每個 8-15 字，以動詞開頭。
    - actionMorality: (陣列) **必填**。恰好 3 個數字，對應 actionOptions 的善惡值。
      **【鐵律 — 三選項必須包含道德衝突】**
      你必須按以下模式設計三個選項，每個選項同時決定文字內容和善惡值：
        (1) **善選項**：幫助他人、行俠仗義、保護弱者、誠實正直 → actionMorality 填正數（5~15）
        (2) **惡選項**：偷竊、欺騙、傷害無辜、趁火打劫、威脅恐嚇 → actionMorality 填負數（-5~-15）
        (3) **中或任意**：觀察、移動、探索、或再一個善/惡 → actionMorality 填 0 或正負數
      三個選項的順序可以隨機打亂，但**必須至少包含一個正數和一個負數**。禁止三個都是 0。
      範例：actionOptions: ["出手救下被欺負的少女", "趁亂偷走攤販的錢袋", "默默離開不管閒事"], actionMorality: [10, -10, 0]
