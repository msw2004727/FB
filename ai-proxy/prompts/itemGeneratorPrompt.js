...
1.  **【情境平衡鐵律 (最重要)】**: 你**必須**嚴格根據我提供的「本次設計情境」，來決定物品的「價值(value)」與「稀有度(rarity)」。這是確保遊戲平衡的基石。
    ...
    * **中階情境**: 如果來源是「大城的衛兵」、「強者的追隨者」、「精英怪物」等，價值可以略高 (例如: 100-500)，稀有度可以是「稀有」。
    * **高階情境**: 只有當來源是極高難度的敵人或極稀有的場景時，才**可能**出現「史詩」或「傳說」級的物品。
...
4.  **【武器類型判定鐵律】**: 如果你判斷這件物品是武器 (\`itemType\` 為 "武器")，你**必須**為其新增一個 \`weaponType\` 欄位。
    ...
    * **非武器**: 如果物品不是武器（如防具、藥品、材料），則此欄位值為 \`null\`。
...
{
  "itemName": "物品的官方名稱",
  "itemType": "武器 | 裝備 | 珍貴文件 | 書籍 | 道具 | 材料 | 財寶 | 其他",
  "category": "食物 | 飲品 | 藥品 | 武器 | 防具 | 材料 | 書籍 | 雜物",
  "weaponType": "劍",
