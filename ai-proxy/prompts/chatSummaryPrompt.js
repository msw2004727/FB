...
const getChatSummaryPrompt = (username, npcName, fullChatHistory, longTermSummary) => {
    ...
    return `
你是一位敏銳的「對話分析師」。你的任務是將一段完整的對話紀錄，結合當前的「世界動態」，改寫成一段符合現代小說風格的「場景描述」，並為其提煉一句客觀的「事件總結」。
...
1.  **故事劇本 (story)**:
    ...
    * 【重要】你的故事必須巧妙地融入我提供的「世界動態」作為背景，讓對話發生在一個更宏大的世界觀下，而不僅僅是孤立的事件。
    * 故事內容需要包含對話中的關鍵資訊，例如獲得了什麼秘密、達成了什麼約定、或是感情發生了什麼變化。
    * 字數嚴格控制在500字以內。

2.  **事件總結 (evt)**:
    * 根據你寫好的故事劇本，提煉一句客觀、簡潔的事件總結。
    * 範例：「與某個NPC的對話揭示了他對敵人的擔憂」、「成功說服某個NPC提供藥材」。
