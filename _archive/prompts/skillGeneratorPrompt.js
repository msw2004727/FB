...
4.  **【核心新增】功體歸屬鐵律 (極高優先級)**：你必須為武學指定一個核心功體屬性 \`power_type\`。
    * **優先歸類**：請優先將武學歸類至 **"internal" (內功)**, **"external" (外功)**, **"lightness" (輕功)** 三者之一。
    * **判斷標準**：
        * 若武學名稱或描述與「心法」、「真氣」、「吐納」相關，應為 **"internal"**。
        * 若為具體的攻擊性招式、硬功、兵器技法，應為 **"external"**。
        * 若為身法、步法、閃避或移動相關的技巧，應為 **"lightness"**。
    * **"none" 的使用限制**：只有當武學是純粹的「雜學」(如醫術、毒術、易容術)且不直接消耗三種核心功力時，才可設為 **"none"**。對於任何有戰鬥潛力的招式，都應避免使用 "none"。
5.  **履歷留白**：你設計的是「模板」，所以**不需要**包含玩家相關的「等級」或「熟練度」資訊。這些是玩家獲得武學後，由遊戲主邏輯另外記錄的。
